...
class Faction(object):
    def __init__(self, num, name):
        ...
        self.validators = {Validator(str("%s_%d") % (
            name, k), self) for k in range(num)}
...
# , flipped=False):
def attestation_honest_majority(validator, time_current, factions, votes, delay_param):
    """The honest way of voting
    This functionality does not give tiebreaker.
    Instead, when the number of votes for 0 and 1 are tie, an honest validator choose one in a ramdonly manner.
    Args:
    ...
    for (v, timing, vote) in votes:
        ...
        logstr = "  %s voted for %s [t=%.3f; t_received=%.3f]" % (
            v.name, str(vote), timing, time_received)
    ...
    if seen_votes[0] > seen_votes[1]:
        vote = 0
    else if seen_votes[0] < seen_votes[1]:
        vote = 1
    else:
        random.choice([0, 1])
    # if flipped:
